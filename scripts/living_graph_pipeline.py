...
def _to_ollama_options() -> Dict[str, Any]:
    ...
    reasoning_effort = (
        os.getenv("OLLAMA_REASONING_EFFORT") or os.getenv("LLM_REASONING_EFFORT") or ""
    ).strip()
    if reasoning_effort:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Write raw_extractions_general.partial.json after each processed chunk",
    )
    p.add_argument(
        "--resume-extractions",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Resume extraction from raw_extractions_general.partial.json when present",
    )
...
async def _run(args: argparse.Namespace) -> int:
    ...
    checkpoint_path = out_dir / "raw_extractions_general.partial.json"
    parsed_chunks: List[Dict[str, Any]] = []
    if args.resume_extractions and checkpoint_path.exists():
        try:
            existing = json.loads(checkpoint_path.read_text(encoding="utf-8"))
            if isinstance(existing, list):
                parsed_chunks = [x for x in existing if isinstance(x, dict)]
                print(f"  resume_checkpoint={checkpoint_path} chunks_done={len(parsed_chunks)}")
        except Exception as e:
            print(f"  resume_checkpoint=ignored ({e})")

    for idx, ch in enumerate(chunks, start=1):
        if idx <= len(parsed_chunks):
            prev = parsed_chunks[idx - 1]
            if int(prev.get("chunk_id") or -1) == ch.chunk_id and list(prev.get("pages") or []) == ch.page_indices:
                print(f"  - chunk {idx}/{len(chunks)} pages={ch.page_indices} (checkpoint)")
                continue
            raise RuntimeError(
                "Extraction checkpoint does not match current chunking; "
                "remove partial checkpoint or rerun with matching chunk args"
            )
        ...
        if args.checkpoint_extractions:
