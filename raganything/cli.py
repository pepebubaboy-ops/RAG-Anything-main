...
from .genealogy.models import Claim, Evidence, PersonSpec
from .genealogy.stores import InMemoryGenealogyStore
    ...
    *,
    ...
    for pattern in _PARENT_PATTERNS:
        for match in pattern.finditer(compact_text):
            child = _clean_name(match.group("child"))
            parent1 = _clean_name(match.group("parent1"))
            parent2 = _clean_name(match.group("parent2") or "")
            ...
            if parent2:
                parents.append({"name": parent2})

            yield Claim(
                claim_type="parent_child",
                confidence=0.8,
                data={"parents": parents, "child": {"name": child}},
                evidence=[
    ...
    for pattern in _SPOUSE_PATTERNS:
        for match in pattern.finditer(compact_text):
            person1 = _clean_name(match.group("person1"))
            person2 = _clean_name(match.group("person2"))
            ...
            yield Claim(
                claim_type="spouse",
                confidence=0.75,
                data={"person1": {"name": person1}, "person2": {"name": person2}},
                evidence=[
                    Evidence(file_path=source, page_idx=page_idx, quote=match.group(0))
                ],
            )
...
def _apply_claim_to_store(store: InMemoryGenealogyStore, claim: Claim) -> bool:
    if claim.claim_type == "parent_child":
        ...
        child_rec = store.upsert_person(PersonSpec(name=child_name))
        ...
        for parent in parents_payload[:2]:
            ...
            if not parent_name:
                continue
            parent_rec = store.upsert_person(PersonSpec(name=parent_name))
    ...
    if claim.claim_type == "spouse":
        p1_name = _clean_name((claim.data.get("person1") or {}).get("name") or "")
        p2_name = _clean_name((claim.data.get("person2") or {}).get("name") or "")
        if not p1_name or not p2_name:
            return False

        p1 = store.upsert_person(PersonSpec(name=p1_name))
        p2 = store.upsert_person(PersonSpec(name=p2_name))
...
def _pair_has_merge_anchor(pair_link: Dict[str, Any] | None) -> bool:
    ...
    if pair_link.get("max_confidence", 0.0) >= 0.9:
        return True
    if pair_link.get("support_count", 0) >= 2 and relation_types & supported_types:
        return True
    return False
    ...
    *,
    ...
    if left_features["first_token"] and left_features["first_token"] == right_features["first_token"]:
        ...
        if max(left_features["token_count"], right_features["token_count"]) <= 1:
            return year_anchor or merge_anchor
...
def _cmd_build_living_graph_from_json(args: argparse.Namespace, input_path: Path) -> int:
    ...
    include_relation_types = _parse_relation_types_arg(args.include_relation_types)
    exclude_relation_types = _parse_relation_types_arg(args.exclude_relation_types)
    ...
    print(
        f"Living graph built: entities={len(entities)}, relations={len(relations)}. "
        f"Output: {output_dir}{filter_suffix}"
    )
    return 0
...
def _cmd_genealogy_build(args: argparse.Namespace) -> int:
    ...
    if (
        ...
        and _looks_like_living_graph_json(input_path)
    ):
        return _cmd_build_living_graph_from_json(args, input_path)
    ...
    with claims_path.open("w", encoding="utf-8") as claims_file:
        for source_name, items in sources:
            for claim in _iter_claims_from_content_items(items, source_name):
                applied = _apply_claim_to_store(store, claim)
                claims_file.write(
                    json.dumps(
                        _claims_to_jsonl_row(claim, applied),
                        ensure_ascii=False,
                    )
                    + "\n"
                )
    ...
    print(
        f"Build complete: people={len(people)}, families={len(families)}, claims={total_claims}. "
        f"Output: {output_dir}"
    )
    return 0
...
def _cmd_genealogy_export(args: argparse.Namespace) -> int:
    ...
    if args.format == "dot":
        ...
        print(f"DOT exported: {output_path}")
        return 0
    ...
    if args.format == "gedcom":
        ...
        print(f"GEDCOM exported: {output_path}")
        return 0
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="raganything",
        description="Offline-first genealogy and optional multimodal tooling.",
    )
    parser.add_argument(
        "--version",
        action="version",
